...
/**
 * Class tasksMigrateWebasystremoteTransport
 * @title WebAsyst Tasks & Projects (old version) on a remote server
 * @description Migrate works and tasks
 */
class tasksMigrateWebasystremoteTransport extends tasksMigrateWebasystTransport
{
    ...
    protected function initOptions()
    {
        ...
        $options = array(
            'url'      => array(
                ...
                'description'  => _wp(
                    'For WebAsyst <strong>PHP software</strong> installed on your server:<br />
1. Download data access script <a href="http://www.webasyst.com/wa-data/public/site/downloads/old-webasyst-export-php.zip">export.php</a>, and upload it to your Webasyst published/ folder via FTP.<br />
2. Enter the complete URL to this file, which should look like this: <strong>http://YOUR_WEBASYST_ROOT_URL/published/export.php</strong><br />
<br />
For <strong>hosted accounts</strong>: get your Secure Data Access URL in your ACCOUNT.webasyst.net backend’s “Account (link in the top right corner) &gt; System Settings” page.'
                ),
    ...
    {
        $url = $this->getURL("file=IT".urlencode(base64_encode($path)));

        $this->log('URL:'.self::logURL($url), self::LOG_DEBUG);
        waFiles::create(dirname($target), true);
        ...
        try {
            ...
            if ($ex->getCode() == 502) {
                ...
                return $this->moveFile($path, $target);
            } else {
    ...
    private function downloadCurl($url, &$target_stream)
    {
        ...
        try {
            ...
            if ($err_no) {
                $url = self::logURL($url);
                $message = "Curl error: {$err_no}# ".curl_error($ch)." at [{$url}]";
