...
class tasksReleasesPluginTaskTypesModel extends waModel
{
    protected $table = 'tasks_releases_task_types';
    const DEFAULT_COLOR = '999';

    public function getAll($key = null, $normalize = false)
    {
        return $this->select("*")->order("sort")->fetchAll($key, $normalize);
    }
    ...
    public function delete($id)
    {
        ...
        $this->deleteById($ids);
        $tem = new tasksReleasesPluginTaskExtModel();
        $tem->updateByField('type', $ids, array(
            'type' => null
        ));
        $tem->cleanEmptyRows();
    }
